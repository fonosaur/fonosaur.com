...
export default async function FieldNotePage({ params }: { params: { slug: string } }) {
  ...
  return (
    <article style={{ maxWidth: 680, margin: "0 auto", padding: "70px 22px 120px", color: C.text, fontFamily: "system-ui, -apple-system, sans-serif" }}>
      <Link href="/" style={{ color: C.sub, fontSize: 13, textDecoration: "none" }}>← Back</Link>

      <div style={{ fontFamily: "monospace", fontSize: 11, color: C.blue, letterSpacing: "0.12em", margin: "22px 0 8px" }}>
        {fmt(note.publishedAt)}
        {note.tags?.length ? ` · ${note.tags.join(" · ")}` : ""}
      </div>
      <h1 style={{ fontWeight: 300, fontSize: "clamp(28px,6vw,44px)", margin: "0 0 20px", lineHeight: 1.1 }}>{note.title}</h1>

      {note.cover && (
        // eslint-disable-next-line @next/next/no-img-element
        <img src={note.cover} alt="" style={{ width: "100%", borderRadius: 14, border: `1px solid ${C.line}`, marginBottom: 22 }} />
      )}

      {note.summary && <p style={{ fontSize: 17, lineHeight: 1.6, marginBottom: 26 }}>{note.summary}</p>}
